...
AddDuplicateInMemory(HnswElement element, HnswElement dup)
{
	...
	/*
	 * ACORN: append the NEW row's label alongside its heap TID.
	 * element->labels[0] holds the new row's label from InsertTuple.
	 */
...
InsertTuple(Relation index, Datum *values, bool *isnull, ItemPointer heaptid, HnswBuildState * buildstate)
{
	...
	if (graph->flushed)
	{
		...
		return HnswInsertTupleOnDisk(index, support, value, heaptid, true, label);
	}
	...
	/* ACORN: override the default HNSW_LABEL_NONE. */
