...
#define HNSW_VERSION	2          /* ACORN: per-heaptid labels added */
#define HNSW_MAGIC_NUMBER 0xA953A953
#define HNSW_PAGE_ID	0xFF90

/* Sentinel meaning "no label" — never matches a real filter label. */
#define HNSW_LABEL_NONE INT32_MIN
...
/* === ADD before HnswQuery (around line 267) === */
/* LabelHashEntry: removed — labels now live in HnswElementTupleData. */
/*-----------------------change---------------------------*/
// typedef struct HnswQuery
// {
// 	Datum		value;
// }			HnswQuery;
/* === REPLACE HnswQuery struct (line 267-270) === */
typedef struct HnswQuery
{
    Datum       value;
...
/* Added labels in each element tuple */
...
/* ACORN: no auxiliary state needed — labels live in element tuples. */
}			HnswScanOpaqueData;
...
bool		HnswInsertTupleOnDisk(Relation index, HnswSupport * support, Datum value, ItemPointer heaptid, bool building, int32 label);
 ...
 * ACORN helpers.
 *
 * AcornElementMatches      — O(1) walk of element->labels[0..len-1].
 * HnswSearchLayerAcorn     — ACORN-SEARCH-LAYER (paper §5.1 Algorithm 2).
 */
bool    AcornElementMatches(HnswElement element, int32 filter_label);

List *HnswSearchLayerAcorn(char *base, HnswQuery *q, List *ep, int ef,
                           int lc, Relation index, HnswSupport *support,
                           int m, bool inserting, HnswElement skipElement,
                           visited_hash *v, pairingheap **discarded,
                           bool initVisited, int64 *tuples);

extern int   hnsw_acorn_filter_label;
