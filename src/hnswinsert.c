...
AddDuplicateOnDisk(Relation index, HnswElement element, HnswElement dup, bool building)
{
	...
	/*
	 * ACORN: write heaptid and label in lockstep.
	 * `element` is the transient element for the NEW row; its labels[0]
	 * was set by the caller (from the INCLUDE column in values[1]).
	 * We must use THAT label — not the existing element's label, which
	 * belongs to a different heap row with possibly a different value.
	 */
	etup->heaptids[i] = element->heaptids[0];
	etup->labels[i]   = element->labels[0];
	// END ADD ---*/
	/* Commit */
	if (building)
		MarkBufferDirty(buf);
 ...
 */
/* New content: added label parameter for ACORN. */
bool
HnswInsertTupleOnDisk(Relation index, HnswSupport * support, Datum value,
                      ItemPointer heaptid, bool building, int32 label)
{
	...
	element->labels[0] = label; /* ACORN: set label on element for filtered search. */
 ...
 * Insert a tuple into the index
 */
// NEWLY ADDED: added heap relation parameter for ACORN to read label from the heap row.
// static void
...
// }
static void
HnswInsertTuple(Relation index, Datum *values, bool *isnull,
				ItemPointer heaptid)
{
	Datum		value;
	const		HnswTypeInfo *typeInfo = HnswGetTypeInfo(index);
	...
	/*
	 * ACORN: read label from INCLUDE column if present.
	 * CREATE INDEX ... USING hnsw (vec vector_l2_ops) INCLUDE (tag)
	 * makes values[1] = tag. No heap re-fetch needed.
	 */
